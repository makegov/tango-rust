...
- **`contract_appeal` is its own query type**, emitting `alerts.contract_appeal.match`. It overlaps neither `contract` (an award) nor the protest corpus (a challenge to an award): a board decision is a dispute under a contract already awarded, so an alert on one of the three never stands in for another.
